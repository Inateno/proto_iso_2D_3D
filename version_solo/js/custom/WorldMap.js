...
function( DE, MapIso3D )
{
  ...
  {
    chunkSize = chunkSize || {};
    this.chunkSize = { "z": chunkSize.z || 20, "y": chunkSize.y || 20, "x": chunkSize.x || 20 }
  ...
  WorldMap3D.prototype.addEntities = function( entities )
  {
    for ( var i = 0, ent; i < entities.length; ++i )
    {
      ...
      // ent.world = this; // don't need mmmmhh ?
      ent.worldPos   = new DE.Vector2( ent.position.x, ent.position.y, 1 );
      ent.position.x = undefined;
      //ent.chunkPos = local position in the chunk
      //ent.chunkLocation = chunk world position
      // ent.chunkSize  = this.chunkSize;
  ...
  /****
   * getChunk@vector3 ( worldPos@vector3 )
   * return chunk location with a world position
   */
